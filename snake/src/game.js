...
export default class Game {
  ...
  /** @method gameOver
	*Dislays a game over message using the DOM
  */
  gameOver(){
  ...
  */
  ...
  update() {
	if(!this.over){
		...
		//Create food
		if (Math.random() < 0.1) this.food.push(new Food(Math.floor(Math.random() * 100), Math.floor(Math.random() * 100)));
  ...
  render() {
	...
	this.screenBufferContext.drawImage(this.backBufferCanvas);
  }
}
loop(){
	this.update();
